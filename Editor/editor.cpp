...
template <typename T, int array_size = 1> class lazy_ptr {
protected:
...
template <typename T> class lazy_ptr<T, 1> {
protected:
  T _new_val;
...
enum class DragState { None = 0, Begin, End, During };
...
int main() {
  ...
  while (AppEngine::App::EventLoop()) {
    ...
    ImGui::Begin("HSEditor", nullptr, ImGuiWindowFlags_MenuBar);
    if (ImGui::BeginMenuBar()) {
      if (ImGui::BeginMenu("File")) {
        ImGui::MenuItem("Load image...", NULL, nullptr);
        ImGui::MenuItem("Save image...", NULL, nullptr);
        ImGui::Separator();
